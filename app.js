...
app.post('/register', async (req, res) =>{
    ...
    conection.query('INSERT INTO usuarios SET ?',{Name: name, APP: app, APM: apm, username:username, Email:email, Pass: passwordHaash} , (err, result, faild) => {
        if(err) {
            console.log('Huebo un error');
        } else {
            res.render('register', {
                alert: true,
...
app.post('/authe', async (req, res) => {
    ...
    if(user && password) {
        conection.query('SELECT * FROM usuarios WHERE username = ?',[user], async(err, result) => {
...
app.post('/newCita', (req, res) => {
    ...
    try {
        conection.query(`INSERT INTO citas(Nombre_Mascota,Propietario,Telefono,FechaCita,HoraCita,Sintomas) VALUES("${mascota}","${propietario}","${telefono}",'${fecha}','${hora}:00',"${sintomas}")`)
    } catch {
        console.log('Error');
    }
...
app.post('/deletecitas', (req, res) => {
    ...
    conection.query(`DELETE FROM citas WHERE citas.id_cita = ${id}`, (error, result) =>{
        if(error) {
            console.log('Hubo un error')
        } else {
...
app.post('/updatecita', (req, res) => {
    ...
    conection.query(`UPDATE citas SET Nombre_Mascota = "${Nombre_Mascota}", Propietario = "${Propietario}", Telefono = "${Telefono}", FechaCita = '${FechaCita}', HoraCita = '${HoraCita}:00', Sintomas = "${Sintomas}" WHERE id_cita = ${id_cita}`, (error, result) => {
        if(error) {
            console.log('Hubo un erro con update');
        } else {
